...
def Branchadd():
    # if session['lg']=="lin":
         if request.method=="POST":
             ...
             db = Db()
             q=db.insert("insert into login values('','"+email+"','"+str(passwd)+"','branch')")
             db.insert("insert into branch VALUES('"+str(q)+"','"+branch_name+"','"+place+"','"+ district+"','"+ifsc_code+"','"+email+"','"+phone+"','"+branch_code+"')")
             import smtplib
             ...
             try:
                 ...
                 gmail.login('creditcardhelp456@gmail.com', 'lasxsfstdsdxlirf')
             ...
             try:
                 ...
                 return '''<script>alert("SOME THING WENT WRONG");window.location="/adminhome"</script>'''
...
def viewatmrequest():
    db = Db()
    atm=db.select("select * from atm ,customer where atm.c_id=customer.c_id and atm.status='forwarded'")
...
def addcustomer():
    if request.method == "POST":
        ...
        try:
            ...
            gmail.send_message(msg)
            return '''<script>alert("ADD SUCCESSFULLY");window.location="/viewcustomer"</script>'''

        except Exception as e:
            return '''<script>alert("SOME THING WENT WRONG");window.location="/viewcustomer"</script>'''
...
def block(did):
    ...
    db.insert("insert into blockhistory values(null, curdate(), curtime(), '"+did+"')")
...
def transaction():
    db = Db()
    qry=db.select("select transaction.*,customer.*,`type(credit,debit)` as type  from transaction,customer where transaction.customer_id=customer.c_id and  customer.branch_id='"+str(session['lid'])+"'")
...
def addbalance():
    if request.method=="POST":
        ...
        balance=request.form['textfield3']
        db=Db()
        q=db.update("update account set balance=balance+'"+balance+"' where account_no='"+accountno+"' ")
        if q>0:
            q2=db.selectOne("select * from account where account_no='"+accountno+"'")
            db.insert("insert into transaction values('',curtime(),curdate(),'"+balance+"','"+str(q2['c_id'])+"','credit','')")
            return '''<script>alert("BALANCE ADDED SUCCESSFULLY");window.location="/addbalance"</script>'''
        else:
            return '''<script>alert(" INVALID ACCOUNT NUMBER");window.location="/addbalance"</script>'''
    else:
        return render_template("branch/addaccount.html")
...
def andhistory():
    ...
    db = Db()
    res=db.select("select * from blockhistory,atm where blockhistory.atm_id=atm.card_id  and atm.c_id='"+lid+"'")
    if len(res)>0:
        return jsonify(status="ok", data=res)
    else:
...
def and_sendrequest():
    uid=request.form['id']
    db=Db()
    res=db.selectOne("select * from atm where atm.c_id='"+uid+"' and (status='pending' or status='forwarded')")
    if res is not None:
